        ...
        <template #title>
            <div class="flex flex-col">
                <Link :href="route('chat-rooms.index')" class="flex text-gray-800">
                    <icon name="back" class="w-3 h-3 fill-current"></icon>
                    <span class="font-semibold text-lg capitalize ml-2">Trở về</span>
                </Link>
                <div class="flex justify-between items-center mt-5">
                    <h3 class="font-semibold text-lg text-gray-800 leading-tight">
                        {{ room.name }}
                    </h3>
                    <h4 v-if="activeCount > 0">{{ activeCount }} người hoạt động</h4>
                </div>
